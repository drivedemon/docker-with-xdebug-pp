# Start setup environment loan-beta-app

![myImage](https://media.giphy.com/media/NEvPzZ8bd1V4Y/giphy.gif)

Main DB config for loan-beta-app `base on docker config`
```
    - APP_URL={up ot you define in nginx config eg. http://dev-app-loan-beta.peerpower.co.th}
    - ADMIN_URL={up ot you define in nginx config eg. http://dev-admin-loan-beta.peerpower.co.th}
    - DB_HOST=loan-beta-db
    - DB_DATABASE=pp-app-loan-beta
    - DB_USERNAME=admin
    - DB_PASSWORD=1234
    - REDIS_HOST=loan-beta-redis
    - QUEUE_DRIVER=redis
...
Data mart config for loan-beta-app `require data mart app`
```
...
Sendinblue mail server config for loan-beta-app
